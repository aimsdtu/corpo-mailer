...
export type Variant = 'default' | 'subtle' | 'error';
...
const IconContainer = memo(({ children, variant, className = '', theme }: { children: React.ReactNode, variant: 'left'|'center'|'right', className?: string, theme?: Theme }) => (
  <motion.div
...
export const EmptyState = forwardRef<HTMLElement, EmptyStateProps>(({
  ...
  const getTextClasses = (type: 'title'|'description', size: Size, theme: Theme) => {
    const sizes = {
      title: {
        sm: "text-base",
        default: "text-lg",
